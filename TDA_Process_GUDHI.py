...
def topological_process_img(img, dgm=None, tau=None, window_size=None, border_width=None):
    ...
    if dgm is None:
        ...
        # Replace lower_star_img with GUDHI implementation
        cubical_complex = gudhi.CubicalComplex(top_dimensional_cells=img)
        cubical_complex.compute_persistence()
        dgm = cubical_complex.persistence_intervals_in_dimension(0)
        dgm = np.array(dgm)
        
        # Get GUDHI's cofaces (birth-death cell indices)
        regular_cofaces, _ = cubical_complex.cofaces_of_persistence_pairs()
    ...
    for i, idx in enumerate(idxs):
        # Use GUDHI's regular_cofaces for exact birth and death indices
        birth_index, death_index = regular_cofaces[0][idx]
        
        # Convert birth and death indices to image coordinates
        birth_coords = np.unravel_index(birth_index, img.shape, order='F')
        death_coords = np.unravel_index(death_index, img.shape, order='F')
        
        # Set bidx and append to didxs using GUDHI's indices
        bidx = birth_index
        didxs = np.append(didxs, death_index)
        ...
        component_at_idx = component_at_idx == component_at_idx[birth_coords[0], birth_coords[1]]
        if i > 0:
            didxs_in_component = idxs[np.where([component_at_idx[death_coords[0], death_coords[1]] 
                                               for didx in didxs])[0]]
